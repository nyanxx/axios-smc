...
axiosInstance.interceptors.request.use((config) => {
  ...
  console.log(
    `Request sent at ${Date.now()} for URL: "${config.baseURL}${config.url}"`,
  );
  config.headers.Authorization = `Bearer ${token}`;
  const additionData = {
    message: "This object was added through request interceptors",
  };
...
axiosInstance.interceptors.response.use((config) => {
  ...
  console.log("Config:", config);
  const additionData = {
    message: "This object was added through response interceptors",
  };
  return { ...config, additionData };
});
